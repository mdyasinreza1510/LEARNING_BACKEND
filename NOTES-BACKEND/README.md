...
<b>TOKEM BASED AUTENTICATION</b>

   - USER SBSE PEHLE USERNAME, EMAIL , PASSWORD ENETER KREGA THEN SERVER EK TOKEN GENERATE KREGA ISKE BAAD TOKEN SEND HOGA USER KO AUR HR EK REQ ME TOKEN SEND HOGA SERVER SE USER KO HAR REQ K SATH EK TOKEN JAYEGA EG:- 1000 REQ = 1000 TOKEN

   - AB HAM API CREATE KRENGE LEKIN SARE APIS HM EK NAYE FOLDER ME CREATE KRENGE (ROOUTES ME)

   - AB HAM API K LOGICS LIKEHNGE LEKIN ALAG FOLDER (CONTROLLERS) ME 🌚
   LE FOLDER KA DE FOLDER HORHA HAIU BS 
<br>
<b>COOKIES</b>

1) TOKEN KO HAM COOKIES ME SAVE KRWATE HAIN 
2) EXPRESS SERVER ME COKIES KO USE KRNE KELIYE HME EK MIDDLWARE KI ZARURAT PDTI HAI ' COKIE-PARSER '
3) USAGE :- app.use(cookieparser)
4) TOKEN KO COKIE ME SAVE KRWAYEMGE
- res.cokie("token",token)

<b>ENCHANCEMENT IN USER REGISTER</b>

1) BEFORE THIS OUR USERS CAN BE GENERATED MULTIPLE TIMES WITH SAME USERNAME, SAME EMAIL, SO AFTER THIS WE ARE GOING TO ENHANCE THIS SO NO USER CAN BE GENERATED WITH SAME EMAIL AND USERNAME

2) NOW WE'LL LEARN THAT  A USER CAN ONLY MAKE POST WHEN ITS LOGGED IN (goto post.routes)  
- STEP 1:- JIS USER K PAAS TOKEN HOGA ISKA MTLB WO LOGGED IN HAI

3) POST TB HI CEATE KRENGE JAB USER K PASS TOKEN HOGA

 - ISKO CHECK KRE KELIYE EK VAR. ME REQ.COOKIES.TOKEN KO ADD KRENGE FIR IF ELSE STATMENT KA USE KRENGE V
 - FIR HM CHECK KRENGE KI JO TOKEN SE REQ AYI WOH SEVER NE GENERATE KI HAI YA KOI AUR GALAT TOKEN SE REQ AYI HAI AUR WO CHCK HOGA JWT PACKAGE SE EG|:- " jwt.verify(token,process.env.JWT_SECRET) "
